...
import { ArrowLeft, MapPin, Calendar, User, Shield, Clock, ChevronLeft, ChevronRight, ExternalLink, CheckCircle, XCircle, Check, X } from "lucide-react";
...
import { apiService, ApiError, DetailResult, MonumentLieuDetail, MobilierImageDetail, PersonneMoraleDetail, PersonnePhysiqueDetail } from "../config/api";
...
export function ValidateFormDetailPage({ formId, formSource, onBack, onValidated, onSessionExpired, onViewDetail }: ValidateFormDetailPageProps) {
  ...
  const TechnicalInfoItem = ({ label, children, className = "" }: { label: string; children: React.ReactNode; className?: string }) => (
  ...
  const TechnicalSection = ({ title, children, className = "" }: { title: string; children: React.ReactNode; className?: string }) => (
