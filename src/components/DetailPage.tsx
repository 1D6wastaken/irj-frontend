...
export function DetailPage({resultId, onBack, onViewDetail, onEdit, isAuthenticated}: DetailPageProps) {
    ...
    const renderPersonneMoraleSections = () => {
        ...
        const hasTechnicalInfo = result.centuries?.length || personne.simple_mention !== undefined ||
            ...
            result.themes?.length || personne.biens || personne.date_premiere_mention ||
            personne.date_derniere_mention || personne.refondation_date || personne.date_fin ||
            personne.status_text !== undefined || personne.origine_sociale_prof ||
            personne.membres_connus || personne.frequence_reunions || personne.participation_vie_pol ||
            personne.funerailles || personne.autres_fetes || personne.inhumation_costume ||
            personne.fondateurs || personne.statuts || personne.autorisations;
                ...
                <TechnicalSection key="technical" title="Informations techniques">
                    ...
                    {personne.status_text !== undefined && (
                        <TechnicalInfoItem label="Texte des statuts">
                            <div className="flex items-center gap-2">
                                {personne.status_text ? (
                                    <CheckCircle className="w-4 h-4 text-green-500"/>
                                ) : (
                                    <XCircle className="w-4 h-4 text-red-500"/>
                                )}
                                <span>{personne.status_text ? 'Oui' : 'Non'}</span>
                            </div>
                        </TechnicalInfoItem>
                    )}
                    ...
                    {personne.biens && (
                        <TechnicalInfoItem label="Biens">
                            <HtmlContent content={personne.biens}
                                         className="text-sm leading-relaxed bg-muted/30 p-3 rounded-md"/>
                        </TechnicalInfoItem>
                    )}
                    ...
                    {personne.origine_sociale_prof && (
                        <TechnicalInfoItem label="Origine sociale / professionnelle">
                            <HtmlContent content={personne.origine_sociale_prof}
                                         className="text-sm leading-relaxed bg-muted/30 p-3 rounded-md"/>
                        </TechnicalInfoItem>
                    )}
                    ...
                    {personne.membres_connus && (
                        <TechnicalInfoItem label="Membres connus">
                            <HtmlContent content={personne.membres_connus}
                                         className="text-sm leading-relaxed bg-muted/30 p-3 rounded-md"/>
                        </TechnicalInfoItem>
                    )}
                    ...
                    {personne.frequence_reunions && (
                        <TechnicalInfoItem label="Fréquence des réunions">
                            <HtmlContent content={personne.frequence_reunions}
                                         className="text-sm leading-relaxed bg-muted/30 p-3 rounded-md"/>
                        </TechnicalInfoItem>
                    )}
                    ...
                    {personne.participation_vie_pol && (
                        <TechnicalInfoItem label="Participation à la vie politique">
                            <HtmlContent content={personne.participation_vie_pol}
                                         className="text-sm leading-relaxed bg-muted/30 p-3 rounded-md"/>
                        </TechnicalInfoItem>
                    )}
                    ...
                    {personne.funerailles && (
                        <TechnicalInfoItem label="Funérailles">
                            <HtmlContent content={personne.funerailles}
                                         className="text-sm leading-relaxed bg-muted/30 p-3 rounded-md"/>
                        </TechnicalInfoItem>
                    )}
                    ...
                    {personne.autres_fetes && (
                        <TechnicalInfoItem label="Autres fêtes">
                            <HtmlContent content={personne.autres_fetes}
                                         className="text-sm leading-relaxed bg-muted/30 p-3 rounded-md"/>
                        </TechnicalInfoItem>
                    )}
                    ...
                    {personne.inhumation_costume && (
                        <TechnicalInfoItem label="Inhumation / Costume">
                            <HtmlContent content={personne.inhumation_costume}
                                         className="text-sm leading-relaxed bg-muted/30 p-3 rounded-md"/>
                        </TechnicalInfoItem>
                    )}
                    ...
                    {personne.fondateurs && (
                        <TechnicalInfoItem label="Fondateurs">
                            <HtmlContent content={personne.fondateurs}
                                         className="text-sm leading-relaxed bg-muted/30 p-3 rounded-md"/>
                        </TechnicalInfoItem>
                    )}
                    ...
                    {personne.statuts && (
                        <TechnicalInfoItem label="Statuts">
                            <HtmlContent content={personne.statuts}
                                         className="text-sm leading-relaxed bg-muted/30 p-3 rounded-md"/>
                        </TechnicalInfoItem>
                    )}
                    ...
                    {personne.autorisations && (
                        <TechnicalInfoItem label="Autorisations">
                            <HtmlContent content={personne.autorisations}
                                         className="text-sm leading-relaxed bg-muted/30 p-3 rounded-md"/>
                        </TechnicalInfoItem>
                    )}
                </TechnicalSection>
    ...
    const renderMonumentSections = () => {
        ...
        const hasTechnicalInfo = monument.centuries?.length || monument.protected !== undefined ||
            ...
            monument.dimensions || monument.altitude || monument.emplacement ||
            monument.date_construction || monument.premiere_mention ||
        ...
        if (personne.pilgrimage_element) {
            sections.push(
                <TechnicalSection key="pilgrimage" title="Éléments du pèlerinage">
                    <HtmlContent content={personne.pilgrimage_element}
                                 className="text-sm leading-relaxed bg-muted/30 p-3 rounded-md"/>
                </TechnicalSection>
            );
        }
        ...
        if (personne.historiography) {
            sections.push(
                <TechnicalSection key="historiography" title="Historiographie">
                    <HtmlContent content={personne.historiography}
                                 className="text-sm leading-relaxed bg-muted/30 p-3 rounded-md"/>
                </TechnicalSection>
            );
        }
        ...
        if (personne.evenements) {
            sections.push(
                <TechnicalSection key="evenements" title="Événements">
                    <HtmlContent content={personne.evenements}
                                 className="text-sm leading-relaxed bg-muted/30 p-3 rounded-md"/>
                </TechnicalSection>
            );
        }
        ...
        if (personne.preparatifs) {
            sections.push(
                <TechnicalSection key="preparatifs" title="Préparatifs">
                    <HtmlContent content={personne.preparatifs}
                                 className="text-sm leading-relaxed bg-muted/30 p-3 rounded-md"/>
                </TechnicalSection>
            );
        }
        ...
        if (personne.chemin_suivi) {
            sections.push(
                <TechnicalSection key="chemin_suivi" title="Chemin suivi">
                    <HtmlContent content={personne.chemin_suivi}
                                 className="text-sm leading-relaxed bg-muted/30 p-3 rounded-md"/>
                </TechnicalSection>
            );
        }
        ...
        if (personne.arrivee) {
            sections.push(
                <TechnicalSection key="arrivee" title="Arrivée">
                    <HtmlContent content={personne.arrivee}
                                 className="text-sm leading-relaxed bg-muted/30 p-3 rounded-md"/>
                </TechnicalSection>
            );
        }
        ...
        if (personne.retour) {
            sections.push(
                <TechnicalSection key="retour" title="Retour">
                    <HtmlContent content={personne.retour}
                                 className="text-sm leading-relaxed bg-muted/30 p-3 rounded-md"/>
                </TechnicalSection>
            );
        }
        ...
        if (personne.non_execution) {
            sections.push(
                <TechnicalSection key="non_execution" title="Non-exécution">
                    <HtmlContent content={personne.non_execution}
                                 className="text-sm leading-relaxed bg-muted/30 p-3 rounded-md"/>
                </TechnicalSection>
            );
        }
        ...
        if (personne.composition_groupe) {
            sections.push(
                <TechnicalSection key="composition_groupe" title="Composition du groupe">
                    <HtmlContent content={personne.composition_groupe}
                                 className="text-sm leading-relaxed bg-muted/30 p-3 rounded-md"/>
                </TechnicalSection>
            );
        }
